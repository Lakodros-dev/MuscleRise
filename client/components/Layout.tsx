...
const Layout = memo(function Layout({ children }: { children: React.ReactNode }) {
      ...
      if (response.ok) {
        ...
        try {
          ...
          if (usersResponse.ok) {
            ...
            console.warn('Failed to fetch users for admin panel');
            setAdminUsers([]);
          }
        } catch (e) {
  ...
  return (
    <div className="min-h-screen text-foreground relative" style={backgroundStyle} data-theme={state.theme.skin}>
      ...
      <footer className="border-t border-white/10 py-6 text-center text-xs text-foreground/60">
        <span
      ...
      {showAdmin ? (
        <div className="fixed inset-0 z-[105] grid place-items-center bg-black/90 p-4">
                    ...
                    <button 
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (response.ok) {
                            const data = await response.json();
                            setGlobalMuscleBoostEnabled(data.globalMuscleBoostEnabled);
                            // Broadcast the change to other components
                            broadcastGlobalMuscleBoostChange(data.globalMuscleBoostEnabled);
                            // Show a confirmation message
                            alert(`Muscle Boost globally ${data.globalMuscleBoostEnabled ? 'enabled' : 'disabled'} successfully!`);
                          } else {
                            console.error('Failed to toggle global muscle boost');
                          }
                        } catch (e) {
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-[110] grid place-items-center bg-black/90 p-4">
