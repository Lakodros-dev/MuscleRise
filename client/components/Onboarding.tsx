  ...
  const register = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg = err.error || "Register failed";
      ...
      dispatch({ type: "HYDRATE", payload: { user: data.user } });
      dispatch({ type: "SELECT_PLAN", payload: { planId: form.planId } });
      try { localStorage.setItem("mr_app_state_v1", JSON.stringify({ ...state, user: data.user })); } catch (e) { }
      setShowWelcome(true);
