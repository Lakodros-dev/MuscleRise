...
const Settings = memo(function Settings() {
  ...
  useEffect(() => {
    if (state.user?.id) {
      ...
      // Fetch user exercise statistics
      fetch(`/api/auth/user/${state.user.id}/exercises`)
        .then(res => res.json())
        .then(data => {
          setUserExerciseStats(data);
        })
        .catch(e => {
          if (process.env.NODE_ENV === 'development') {
            console.warn('Failed to load exercise stats:', e);
          }
        });
    }
  }, [state.user?.id]);
  ...
  const handleLogout = useCallback(() => {
    ...
    // Use a small delay to ensure localStorage is updated before reload
    setTimeout(() => {
  ...
  const addExerciseToTemp = async () => {
    ...
    if (state.user?.id) {
      try {
        ...
        const userResponse = await fetch(`/api/auth/user/${state.user.id}`);
        const userData = await userResponse.json();
        const existingExercises = userData.user.selectedExercises || [];
        ...
        await fetch(`/api/auth/user/${state.user.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ selectedExercises: updatedExercises }),
        });
      } catch (e) {
        if (process.env.NODE_ENV === 'development') {
  ...
  const saveCustomPlan = async () => {
    ...
    if (state.user?.id) {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn('Failed to save custom plan to server:', e);
        }
      }
    }
    
    dispatch({ type: "SELECT_PLAN", payload: { planId: "custom-plan" } });
      ...
      <div className="w-full min-h-screen px-2 py-4">
        ...
        <div className="mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
          ...
          <select
            ...
            onChange={(e) => {
              if (e.target.value === "custom-plan") {
                setIsCustomPlanModalOpen(true);
              } else {
                dispatch({ type: "SELECT_PLAN", payload: { planId: e.target.value } });
              }
            }}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
          >
