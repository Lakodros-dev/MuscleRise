...
const ShopPage = memo(function ShopPage() {
  ...
  useEffect(() => {
    const fetchGlobalSettings = async () => {
      try {
        ...
        const response = await makeApiRequest('/admin/info', {}, true); // Use cache
  ...
  const items = useMemo((): ShopItem[] => {
    const baseItems: ShopItem[] = [
      ...
      { id: "bg-gradient", name: translateThemeName("Background Gradient", t), cost: 600, type: "skin" },
      { id: "bg-forest", name: translateThemeName("Forest Theme", t), cost: 600, type: "skin" },
    ];
    ...
    if (state.user?.id) {
      try {
        ...
        if (response.success) {
          ...
          console.warn('❌ Failed to save custom primary color - server response:', response.error);
        }
      } catch (e) {
  ...
  return (
    <Layout>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-semibold text-foreground">{t.shop.title}</h2>
        <div className="text-sm text-foreground">{t.common.coins}: <span className="font-semibold" style={{ color: "rgb(var(--primary-rgb))" }}>{state.coins}</span></div>
      </div>

      <div className="rounded-xl border border-border bg-card p-4 mb-4 relative z-0">
        <h3 className="font-semibold mb-2 text-foreground">{t.shop.primaryColors} ({t.shop.costCoinsEachTime})</h3>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            ...
            <span className="text-sm" style={{ color: `rgb(var(--primary-rgb))` }}>{t.shop.colorPicker}</span>
          </button>
        </div>
      </div>
